...
import { useAllCategories, subCategoriesApi } from '../api/categories.api'
...
export const SubCategoryForm = ({ initialData, isEdit, onSubmit, isLoading }: SubCategoryFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {!isEdit && (
        <>
          ...
          <div>
            ...
            <input name="sub_category_code" value={formData.sub_category_code} onChange={handleChange}
              className={`${inputClass} ${isDuplicateCode ? 'border-red-500' : ''}`} required />
            {isDuplicateCode && <p className="text-red-500 text-sm mt-1">Kode sudah digunakan untuk kategori ini</p>}
          </div>
        </>
      )}
      <div>
      ...
      <button type="submit" disabled={isLoading || isDuplicateCode}
