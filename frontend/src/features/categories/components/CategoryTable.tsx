...
import { Eye, Pencil, Trash2, RotateCcw, FolderOpen } from 'lucide-react'
    ...
    return (
      <div className="p-12">
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">Tidak ada kategori</h3>
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {categories.map(cat => (
            <tr key={cat.id} onClick={() => onView(cat.id)} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition cursor-pointer">
              ...
              <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">{cat.category_name}</td>
              ...
              <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-end gap-1">
                  {showDeleted ? (
                    <button
                      onClick={() => onRestore(cat.id, cat.category_name)}
                    ...
                    <>
                      ...
                      <button
                        onClick={() => onDelete(cat.id, cat.category_name)}
