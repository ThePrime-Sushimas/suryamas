...
type PcStatusFilter = 'PENDING' | 'DISBURSED' | 'CLOSED' | 'REJECTED' | ''
const VALID_STATUSES = new Set<PcStatusFilter>(['PENDING', 'DISBURSED', 'CLOSED', 'REJECTED', ''])

export type PettyCashFilters = UrlFilterBase & {
  status: PcStatusFilter
  branch_id: string
  date_from: string
  date_to: string
  search: string
...
export const pettyCashFilterConfig: UrlFilterUtils<PettyCashFilters> & { searchField: 'search' } = {
