        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            {item.physical_count !== null && (
              <>
                <div><span className="text-gray-400">Pecahan Besar</span><p className="font-mono text-gray-900 dark:text-white">{fmt(item.large_denomination || 0)}</p></div>
                <div><span className="text-gray-400">Pecahan Kecil</span><p className="font-mono text-gray-900 dark:text-white">{fmt(item.small_denomination || 0)}</p></div>
                <div><span className="text-gray-400">Total Fisik</span><p className="font-mono font-semibold text-gray-900 dark:text-white">{fmt(item.physical_count)}</p></div>
                <div>
                  <span className="text-gray-400">Selisih</span>
                  <p className={`font-mono font-semibold ${(item.difference || 0) > 0 ? 'text-blue-600' : (item.difference || 0) < 0 ? 'text-red-600' : 'text-green-600'}`}>
          ...
          {/* Closed */}
          {item.status === 'CLOSED' && (
            <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl text-xs text-green-700 dark:text-green-300 text-center">
