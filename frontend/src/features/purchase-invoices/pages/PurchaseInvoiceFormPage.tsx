...
export default function PurchaseInvoiceFormPage() {
  ...
  const { data: existingPI, isLoading: isFetchingPI } = usePurchaseInvoice(isEdit ? id : '')
  const { data: suppliersData } = useSuppliers({ limit: 100 })
  const { data: branchesData } = useBranches({ limit: 100 })
  const { data: availableGrs, isLoading: isFetchingGrs } = useAvailableGrs(supplierId, branchId)
  ...
  useEffect(() => {
    if (isEdit && existingPI) {
      ...
      setLines(existingPI.lines.map(l => ({
        ...
        gr_number: existingPI.gr_links.find(gl => existingPI.lines.some(pl => pl.gr_line_id === l.gr_line_id))?.goods_receipt_number ?? '—'
      })))
    }
  }, [isEdit, existingPI])
  ...
  const handleGrToggle = async (grId: string) => {
    if (selectedGrIds.includes(grId)) {
      ...
      if (grData) {
        setLines(prev => prev.filter(l => l.gr_number !== grData.gr_number))
      }
    } else {
      try {
  ...
  if (isEdit && isFetchingPI) return <div className="p-8 text-center">Loading...</div>
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 lg:px-6 py-4 sticky top-0 z-10">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg lg:text-xl font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit Invoice' : 'Buat Invoice Baru'}</h1>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Supplier</label>
                <select 
                  value={supplierId} 
                  onChange={e => { setSupplierId(e.target.value); setSelectedGrIds([]); setLines([]) }}
                  disabled={isEdit}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Cabang</label>
                <select 
                  value={branchId} 
                  onChange={e => { setBranchId(e.target.value); setSelectedGrIds([]); setLines([]) }}
                  disabled={isEdit}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-indigo-500 outline-none disabled:opacity-50"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Nomor Invoice Supplier</label>
                <input 
                  type="text" 
                  value={invoiceNumber} 
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tanggal Invoice</label>
                <input 
                  type="date" 
                  value={invoiceDate} 
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Catatan</label>
              <textarea 
                value={notes} 
          ...
          <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
            <h2 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Pilih Penerimaan Barang (GR)</h2>
            {!supplierId || !branchId ? (
              ...
              <div className="space-y-2 max-h-60 overflow-auto pr-2 no-scrollbar">
                {availableGrs?.map(gr => (
                  <label key={gr.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors">
                    <input 
                      type="checkbox" 
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-gray-500">{new Date(gr.received_date).toLocaleDateString()}</div>
                    </div>
                  </label>
                ))}
                {isEdit && existingPI?.gr_links.map(gl => (
                  <div key={gl.goods_receipt_id} className="flex items-center gap-3 p-2.5 rounded-lg border border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/30 dark:bg-indigo-900/10">
                    <CheckCircle2 className="w-4 h-4 text-indigo-600" />
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-gray-900 dark:text-white truncate">{gl.goods_receipt_number}</div>
                      <div className="text-[10px] text-gray-500">{new Date(gl.received_date).toLocaleDateString()}</div>
                    </div>
                  </div>
                ))}
              </div>
                      ...
                      <tr key={index} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          <input 
                            ...
                            onChange={e => handleLineChange(index, { qty_invoiced: Number(e.target.value) })}
                        ...
                        <td className="px-4 py-3 text-right">
                          ...
                          />
                          <div className="text-[10px] text-gray-400 mt-0.5">PO: {fmtCurrency(l.unit_price_po)}</div>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <input 
                            type="number" 
                            value={l.tax_rate}
                            onChange={e => handleLineChange(index, { tax_rate: Number(e.target.value) })}
          ...
          <div className="bg-gray-50/50 dark:bg-gray-700/30 px-6 py-4 border-t border-gray-100 dark:border-gray-700">
            <div className="flex flex-col items-end gap-2">
              <div className="flex justify-between w-64 text-sm">
                ...
                <span className="font-medium text-gray-900 dark:text-white">{fmtCurrency(totals.subtotal)}</span>
              </div>
              <div className="flex justify-between w-64 text-sm">
                <span className="text-gray-500">Total PPN</span>
                <span className="font-medium text-gray-900 dark:text-white">{fmtCurrency(totals.tax)}</span>
              </div>
              <div className="flex justify-between w-64 pt-2 border-t border-gray-200 dark:border-gray-600 font-bold text-lg">
                <span className="text-indigo-600 dark:text-indigo-400">Total Akhir</span>
                <span className="text-indigo-600 dark:text-indigo-400">{fmtCurrency(totals.total)}</span>
              </div>
            </div>
          </div>
        </div>
