        ...
        : inp.requires_processing
          ? inp.output_template.length > 0
            ? inp.output_template.map((t, i) => ({
                ...
                qty_output: t.suggested_pct != null
...
const STATUS_CONFIG = {
  ...
  PROCESSING: { label: "Diproses",  color: "bg-blue-50 text-blue-700",    dot: "bg-blue-500"  },
  QC_REVIEW:  { label: "Review QC", color: "bg-yellow-50 text-yellow-700",dot: "bg-yellow-500"},
  CONFIRMED:  { label: "Selesai",   color: "bg-green-50 text-green-700",  dot: "bg-green-500" },
  REJECTED:   { label: "Ditolak",   color: "bg-red-50 text-red-700",      dot: "bg-red-500"   },
};
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-semibold text-gray-900 text-base truncate">{input.product_name}</p>
      ...
      <div className="flex items-center gap-2 flex-wrap border-t border-gray-100 pt-3">
        ...
        {output.flagged_for_return && (
          <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-lg px-2.5 py-1.5">
            <span className="text-xs text-orange-700 font-medium">🔄 Retur</span>
          </div>
        )}
        {output.is_waste && (
          <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 rounded-lg px-2.5 py-1.5">
            <span className="text-xs text-red-700 font-medium">🗑 Waste</span>
          </div>
        )}
        {isDone && (
          <div className="flex items-center gap-1.5 bg-green-100 border border-green-300 rounded-lg px-2.5 py-1.5">
            <span className="text-xs text-green-700 font-bold">✓ Masuk gudang</span>
          </div>
        )}
      </div>
...
function DisassemblyOutputRow({
  ...
  return (
    <div className={`rounded-lg border p-3 space-y-2 ${output.is_waste ? "border-red-200 bg-red-50/30" : "border-gray-200 bg-white"}`}>
      <div className="flex items-center gap-2">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-gray-800 truncate">{output.product_name}</p>
          <p className="text-xs text-gray-400 font-mono">{output.product_code}</p>
        </div>
        {isEditable && (
      ...
      {isEditable ? (
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm font-semibold text-gray-800">{output.qty_output}</span>
          <span className="text-xs text-gray-500">{output.uom}</span>
          {output.is_waste && <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full">waste</span>}
          {output.stock_movement_id && <span className="text-xs bg-green-100 text-green-600 px-1.5 py-0.5 rounded-full">✓ gudang</span>}
        </div>
      )}
    </div>
  )
...
function DisassemblyCard({
  ...
  return (
    <div className={`rounded-xl border-2 p-4 space-y-3 transition-all ${isDone ? "border-green-300 bg-green-50/30" : "border-gray-200 bg-white"}`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 border ${
                overLimit ? "bg-red-50 border-red-300" : pct >= 90 ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-200"
              }`}>
                <span className={`w-2 h-2 rounded-full shrink-0 ${overLimit ? "bg-red-500" : pct >= 90 ? "bg-green-500" : "bg-gray-400"}`} />
                <span className={`text-xs font-medium ${overLimit ? "text-red-700" : pct >= 90 ? "text-green-700" : "text-gray-600"}`}>
  ...
  return (
    <div className="rounded-xl border-2 border-orange-200 bg-orange-50/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-orange-800 text-sm">Menunggu Retur ({returnItems.length})</h3>
      </div>
      {returnItems.map(item => (
        <div key={item.id} className="bg-white rounded-lg border border-orange-200 p-3 space-y-2">
          <div className="flex justify-between items-start gap-2">
            <div>
              <p className="text-sm font-medium text-gray-800">{item.product_name}</p>
              {item.return_reason && <p className="text-xs text-gray-500 mt-0.5">"{item.return_reason}"</p>}
            </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl">
        ...
        />
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors">Batal</button>
              ...
              {template.map(t => (
                <button key={t.id} type="button"
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all ${
                    productId === t.output_product_id ? "border-blue-400 bg-blue-50 text-blue-800" : "border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <span className="font-medium">{t.output_product_name}</span>
                  <span className="text-gray-400 ml-2 text-xs">{t.output_uom}</span>
...
export default function GoodsProcessingDetailPage() {
  ...
  const handleConfirmItem = useCallback(async (inp: LocalInput) => {
    ...
    try {
      ...
      addToast("success", `${inp.product_name} masuk gudang ✓`)
    } catch (e) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-20">
        <div className="flex items-center gap-3 px-4 py-3">
          <button onClick={() => navigate(-1)} className="p-1.5 -ml-1.5 text-gray-500 hover:text-gray-800 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-gray-900 text-sm truncate">{gp.processing_number}</p>
            <p className="text-xs text-gray-500 truncate">{gp.supplier_name} · {gp.gr_number}</p>
          </div>
        ...
        {status === "PROCESSING" && totalCount > 0 && (
          <div className="px-4 pb-3">
            <div className="flex items-center justify-between text-xs mb-1.5">
              <span className="text-gray-500">Progress item</span>
      ...
      <div className="lg:flex lg:gap-0 lg:min-h-[calc(100vh-64px)]">
        ...
        <div className="lg:w-80 lg:shrink-0 lg:border-r lg:border-gray-200 lg:bg-white lg:sticky lg:top-16 lg:h-[calc(100vh-64px)] lg:overflow-y-auto">
          <div className="p-4 space-y-4">

            {/* Info card */}
            <div className="bg-white lg:bg-gray-50 rounded-xl border border-gray-100 lg:border-gray-200 p-3.5 grid grid-cols-2 gap-3">
              <div>
                <p className="text-xs text-gray-400">Gudang</p>
                <p className="font-medium text-gray-800 text-xs mt-0.5">{gp.warehouse_name}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-gray-800 text-xs mt-0.5">{gp.branch_name}</p>
              </div>
            </div>
            ...
            {status === "CONFIRMED" && gp.total_input_qty != null && (
              <div className="bg-green-50 rounded-xl border border-green-200 p-4 space-y-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="font-semibold text-green-800 text-sm">Ringkasan Proses</p>
                </div>
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div>
                    <p className="text-lg font-bold text-gray-800">{gp.total_input_qty}</p>
                    <p className="text-xs text-gray-500">Input</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-green-700">{gp.total_output_qty}</p>
                    <p className="text-xs text-gray-500">Output</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-gray-600">{gp.yield_percentage}%</p>
            ...
            <div className="hidden lg:block space-y-2">
              ...
              {(status === "PROCESSING" || status === "REJECTED") && isEditable && (
                <div className="flex gap-2">
                  <button onClick={handleSave} disabled={isBusy}
                    className="flex items-center gap-1.5 px-4 py-3 border-2 border-gray-200 text-gray-700 rounded-xl font-medium text-sm disabled:opacity-50 hover:border-gray-300 transition-all shrink-0"
                  >
                    <Save size={15} />
                    Simpan
                  </button>
                  {canApprove && (
                    <button onClick={() => setShowRejectModal(true)} disabled={isBusy}
                      className="flex items-center gap-1.5 px-4 py-3 border-2 border-red-200 text-red-600 rounded-xl font-medium text-sm disabled:opacity-50 hover:bg-red-50 transition-all shrink-0"
                    >
                      <XCircle size={15} />
                      Tolak
                    </button>
              ...
              {status === "QC_REVIEW" && canApprove && (
                <div className="flex gap-2">
                  <button onClick={() => setShowRejectModal(true)} disabled={isBusy}
                    className="flex-1 py-3 border-2 border-red-200 text-red-600 rounded-xl font-semibold text-sm disabled:opacity-50 hover:bg-red-50 transition-all"
                  >Tolak</button>
              ...
              {status === "CONFIRMED" && (
                <div className="flex items-center justify-center gap-2 py-2">
                  <CheckCircle2 size={16} className="text-green-600" />
                  <p className="text-sm text-green-700 font-medium">Proses selesai</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 safe-area-pb">
        <div className="max-w-lg mx-auto">
          ...
          {status === "CONFIRMED" && (
            <div className="flex items-center justify-center gap-2 py-2">
              <CheckCircle2 size={16} className="text-green-600" />
              <p className="text-sm text-green-700 font-medium">Proses selesai · Stok sudah masuk gudang</p>
            </div>
          )}
        </div>
      </div>
