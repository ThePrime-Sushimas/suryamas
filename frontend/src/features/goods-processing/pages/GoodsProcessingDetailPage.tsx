      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 px-4 py-3 safe-area-pb">
        <div className="max-w-lg mx-auto">
          ...
          {(status === "PROCESSING" || status === "REJECTED") && isEditable && (
            <div className="flex gap-2">
              <button onClick={handleSave} disabled={isBusy}
                className="flex items-center gap-1.5 px-4 py-3.5 border-2 border-gray-200 text-gray-700 rounded-2xl font-medium text-sm disabled:opacity-50 hover:border-gray-300 dark:hover:border-gray-500 transition-all shrink-0"
              >
                <Save size={15} />
                Simpan
              </button>
              {canApprove && (
                <button onClick={() => setShowRejectModal(true)} disabled={isBusy}
                  className="px-3 py-3.5 border-2 border-red-200 text-red-600 rounded-2xl font-medium text-sm disabled:opacity-50 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all shrink-0"
                >
                  <XCircle size={15} />
                </button>
              )}
