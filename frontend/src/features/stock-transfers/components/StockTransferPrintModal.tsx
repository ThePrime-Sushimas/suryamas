...
import { usePrinters, usePrintStockTransfer } from '@/features/printers/api'
import type { StockTransferLine } from '../api/stockTransfers.api'
...
export function StockTransferPrintModal({ transferId, lines, onClose }: Props) {
  const toast = useToast()
  const { data: printers = [], isLoading: loadingPrinters } = usePrinters()
  const printMutation = usePrintStockTransfer()

  const activePrinters = printers.filter(p => p.is_active)
  const defaultPrinter = activePrinters.find(p => p.is_default)

  const printableLines = lines.filter((l): l is StockTransferLine & { id: string } => !!l.id)
  ...
  const [selectedLines, setSelectedLines] = useState<Set<string>>(() =>
    new Set(printableLines.map(l => l.id))
  )
              ...
              {printableLines.map(line => (
                ...
                >
                  ...
                  <span className="text-xs text-gray-400">
                    {fmt(Number(line.qty))} {line.base_unit_name ?? ''}
                  </span>
                </label>
              ))}
            </div>
