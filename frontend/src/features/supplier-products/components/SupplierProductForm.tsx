...
export function SupplierProductForm({
  ...
  }, [formData.product_id, initialData, toast])
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
      {/* Supplier Selection */}
      <div>
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Supplier & Product</h3>
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 mt-1 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            >
            ...
            {isEdit && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Supplier cannot be changed</p>}
          </div>
          ...
          <div>
            ...
            {isEdit && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Product cannot be changed</p>}
          </div>
        </div>
      </div>
          ...
          <div>
            <label className="flex items-center cursor-pointer">
              <input
                ...
                className="mr-2 w-4 h-4 text-yellow-500 focus:ring-yellow-500 border-gray-300 dark:border-gray-600 rounded disabled:opacity-50"
              />
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-6">
              ...
              {formData.product_id && (
                <span className={`ml-2 font-medium ${preferredCount >= 3 ? 'text-red-600 dark:text-red-400' : preferredCount >= 2 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'}`}>
                  ({preferredCount}/3 used)
                </span>
              )}
            </p>
            {preferredCount >= 3 && !formData.is_preferred && (
              <p className="text-xs text-red-600 dark:text-red-400 mt-1 ml-6">⚠ Maximum preferred suppliers reached for this product</p>
            )}
          </div>
          <div>
            <label className="flex items-center cursor-pointer">
              <input
                ...
                onChange={handleChange}
                className="mr-2 w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600 rounded"
              />
