    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">{grandTrx} trx</span>
          <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{fmt(grandTotal)}</span>
          <button onClick={onRefresh} disabled={isFetching} className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
          ...
          {grouped.map(([branch, g]) => {
            ...
            return (
              <div key={branch} className="border-b border-gray-50 dark:border-gray-700 last:border-0">
                <button onClick={() => toggle(branch)} className="w-full flex items-center px-4 py-2.5 hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-colors text-left">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <span className="text-[10px] text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-full shrink-0">{g.trx} trx</span>
                  </div>
                ...
                {isOpen && (
                  <div className="px-4 pb-2.5 space-y-0.5">
                    {g.rows.map((row, i) => {
                      ...
                      return (
                      <div key={i} className="flex items-center gap-2 py-1.5 pl-5 rounded-md hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${dotColor}`} />
                        <span className="flex-1 text-xs text-gray-600 dark:text-gray-400 truncate">{row.payment_methods?.name || 'Unknown'}</span>
                        <span className="text-[10px] text-gray-400 shrink-0">{row.transaction_count} trx</span>
