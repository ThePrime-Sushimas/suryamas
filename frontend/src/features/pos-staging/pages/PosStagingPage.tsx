...
export default function PosStagingPage() {
  ...
  useEffect(() => {
    if (isLoaded && hasPermission) fetchRows()
  ...
  useEffect(() => {
    ...
    paymentMethodsApi.getOptions().then(setPaymentMethodOptions).catch(console.error)
  }, [activeTable, hasPermission])
  ...
  const renderTableHeader = () => {
    ...
    return [...base, ...extra[activeTable]].map(h => (
      <th key={h} className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase whitespace-nowrap">
        {h}
      </th>
    ))
  }
  ...
  const renderRow = (row: StagingRow) => {
    ...
    const baseCells = (
      <>
        <td className="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{posId}</td>
        <td className="px-3 py-2 text-sm font-medium text-gray-900 dark:text-gray-100">{name}</td>
        ...
        {isPending ? (
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={isLoading}
              className="px-2 py-1 text-xs bg-gray-400 dark:bg-gray-600 text-white rounded hover:bg-gray-500 dark:hover:bg-gray-500 disabled:opacity-50"
            >
              Ignore
            </button>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50"
          >
            Reset
          </button>
        )}
    ...
    const tdMuted = "px-3 py-2 text-sm text-gray-500 dark:text-gray-400"
    const selectCls = "text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded px-2 py-1"
    const rowCls = "border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50"

    if (activeTable === 'branches') {
      const r = row as StagingBranch
      return (
        <tr key={posId} className={rowCls}>
          {baseCells}
          <td className={tdMuted}>{r.branch_code ?? '—'}</td>
          <td className="px-3 py-2">
            {isPending ? (
              <select
              ...
              <span className="text-sm text-gray-500 dark:text-gray-400">{r.mapped_id ?? '—'}</span>
            )}
          </td>
          {actionButtons}
        </tr>
    ...
    if (activeTable === 'payment_methods') {
      ...
      return (
        <tr key={posId} className={rowCls}>
          ...
          <td className={tdMuted}>{r.coa_no ?? '—'}</td>
          <td className="px-3 py-2">
            {isPending ? (
              <select
                id={`mapping-${activeTable}-${posId}`}
    ...
    if (activeTable === 'menu_categories') {
      ...
      return (
        <tr key={posId} className={rowCls}>
          {baseCells}
          <td className={tdMuted}>{r.sales_coa_no ?? '—'}</td>
          {actionButtons}
        </tr>
      )
    }
    ...
    if (activeTable === 'menu_groups') {
      ...
      return (
        <tr key={posId} className={rowCls}>
          {baseCells}
          <td className={tdMuted}>{r.group_code ?? '—'}</td>
          {actionButtons}
        </tr>
      )
    }
    ...
    return (
      <tr key={posId} className={rowCls}>
        {baseCells}
        <td className={tdMuted}>{r.menu_code ?? '—'}</td>
          ...
          <button
            ...
            className={`px-3 py-1 text-xs rounded-full border transition-colors ${
              statusFilter === s.value
                ...
                : 'text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500'
            }`}
          >
            {s.label}
          </button>
        ))}
        <span className="ml-auto text-sm text-gray-400 dark:text-gray-500">{total} baris</span>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-800/50">
            {loading ? (
              <tr>
                <td colSpan={99} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">
                  Memuat...
                </td>
              </tr>
            ) : rows.length === 0 ? (
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-3 mt-4">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40"
          >
            ← Prev
          </button>
