...
export const opnameFilterConfig: UrlFilterUtils<OpnameFilters> = {
  ...
  parse: (sp) => ({
    ...
    position_id: parseString(sp.get('position_id')),
  ...
  stringify: (f) => {
    ...
    s('position_id', serializeString(f.position_id))
  ...
  merge: (current, patch) =>
    mergeWithPageReset(current, patch, OPNAME_FILTER_DEFAULTS, ['status', 'branch_id', 'position_id', 'date_from', 'date_to', 'search']),
}
