...
const STATUS_OPTIONS: {
  value: BankStatementFilterStatus;
  label: string;
  description: string;
}[] = [
  {
    value: "",
    label: "Semua Status",
    description: "Tampilkan semua transaksi",
  },
  {
    value: "RECONCILED",
    label: "✓ Reconciled",
    description: "Sudah dicocokkan",
  },
  {
    value: "UNRECONCILED",
    label: "○ Unreconciled",
    description: "Belum dicocokkan",
  },
];
...
export const BankReconciliationFilters: React.FC<
  BankReconciliationFiltersProps
> = ({
  filters,
  onFiltersChange,
  onApplyFilters,
  ...
  const handleAccountToggle = (accountId: number) => {
    ...
    const newFilters = {
      ...filters,
      bankAccountIds: updated.length > 0 ? updated : undefined,
    };
  ...
  const handleQuickDateClick = (preset: (typeof presets)[number]) => {
    handleQuickDate(preset.start, preset.end);
  };
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {presets.map((preset) => {
              ...
              return (
                <button
                  key={preset.label}
                  onClick={() => handleQuickDateClick(preset)}
                  className={`inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-all h-auto cursor-pointer border ${
                    isActive
                      ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700"
                      : "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-transparent hover:bg-gray-200 dark:hover:bg-gray-600"
                  }`}
                >
          ...
          {bankAccounts.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {bankAccounts.map((acc) => {
                const isActive =
                  filters.bankAccountIds?.includes(acc.id) || false;
                return (
                  <button
                    key={acc.id}
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap gap-2 mb-4">
              {filters.search && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 dark:bg-blue-900/10 text-blue-700 dark:text-blue-400 rounded-full text-[10px]`}
                >
              ...
              {filters.startDate && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 dark:bg-green-900/10 text-green-700 dark:text-green-400 rounded-full text-[10px]`}
                >
              ...
              {filters.endDate && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400 rounded-full text-[10px]`}
                >
              ...
              {filters.status && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-full text-[10px]`}
                >
                  Status:{" "}
              ...
              {filters.bankAccountIds && filters.bankAccountIds.length > 0 && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-[10px]`}
                >
                  {filters.bankAccountIds.length} Akun dipilih
                  <button
                    onClick={() => handleClearFilter("bankAccountIds")}
                    className="hover:text-indigo-900"
                    aria-label="Hapus akun bank"
                  >
                    <X size={12} />
                  </button>
                </span>
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="flex-1 min-w-[200px]">
              <label
                ...
                className="block text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1"
              >
                Pencarian
              </label>
