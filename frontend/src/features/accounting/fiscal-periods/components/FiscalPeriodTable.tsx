        ...
        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
          {periods.map((period) => (
            <tr key={period.id} className={period.deleted_at ? 'bg-gray-50 dark:bg-gray-800' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}>
              ...
              <td className="px-6 py-4 whitespace-nowrap"><StatusBadge isOpen={period.is_open} /></td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{formatDateTime(period.created_at)}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                {period.deleted_at ? (
                  canUpdate && (
                    <button onClick={() => onRestore(period.id)}
                  ...
                  <>
                    {canUpdate && period.is_open && (
                      <button onClick={() => onEdit(period)}
                        className="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300">
                        Edit
                      </button>
                    )}
                    {canClose && period.is_open && onClose && (
