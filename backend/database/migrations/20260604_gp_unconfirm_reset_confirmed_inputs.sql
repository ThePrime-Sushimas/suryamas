-- After GP finalize, input lines become CONFIRMED (not only DONE).
-- Older unconfirm_goods_processing only reverted DONE → left lines stuck CONFIRMED under CORRECTING header.
...
FROM goods_processing gp
WHERE gp.id = gpi.goods_processing_id
  AND gp.status = 'CORRECTING'
  AND gp.deleted_at IS NULL
  AND gpi.status IN ('DONE', 'CONFIRMED');
