 ...
 *   R2_PUBLIC_URL        — Default public URL prefix (fallback)
 *   R2_PUBLIC_URL_PROFILEPICTURES — Public Dev URL for profilepictures bucket
 *   R2_PUBLIC_URL_BUKTISETORAN    — Public Dev URL for buktisetoran bucket
 *
...
/** Resolve public URL for a specific bucket (per-bucket env > fallback) */
function getPublicBaseUrl(bucket: string): string {
  const envKey = `R2_PUBLIC_URL_${bucket.toUpperCase()}`
  return process.env[envKey] || R2_PUBLIC_URL()
}
...
export const storageService = {
  ...
  async upload(file: Buffer, fileName: string, contentType: string, bucket?: string): Promise<UploadResult> {
    ...
    const publicUrl = getPublicBaseUrl(b) ? `${getPublicBaseUrl(b)}/${path}` : path
    return { path, publicUrl }
  },

  /**
   * Get public URL for a path (bucket-aware)
   */
  getPublicUrl(path: string, bucket?: string): string {
    const base = getPublicBaseUrl(bucket || DEFAULT_BUCKET)
