...
PermissionService.registerModule('accounting-purposes', 'Accounting Purposes Management').catch((error) => {
  console.error('Failed to register accounting-purposes module:', error.message)
})
...
router.get('/', canView('accounting-purposes'), queryMiddleware({
...
router.get('/search', canView('accounting-purposes'), queryMiddleware({
  allowedSortFields: ['purpose_code', 'purpose_name', 'applied_to', 'is_active', 'created_at', 'updated_at', 'id'],
}), (req, res) => 
  accountingPurposesController.search(req as AuthenticatedQueryRequest, res))

// Filter options
router.get('/filter-options', canView('accounting-purposes'), (req, res) => 
  accountingPurposesController.getFilterOptions(req as AuthenticatedRequest, res))

// Export routes with rate limiting
router.get('/export/token', canView('accounting-purposes'), exportLimiter, (req, res) => 
  accountingPurposesController.generateExportToken(req as AuthenticatedRequest, res))

router.get('/export', canView('accounting-purposes'), exportLimiter, (req, res) => 
  accountingPurposesController.exportData(req as AuthenticatedQueryRequest, res))

// Import routes
router.post('/import/preview', canInsert('accounting-purposes'), (req, res) => 
  accountingPurposesController.previewImport(req as AuthenticatedRequest, res))

router.post('/import', canInsert('accounting-purposes'), (req, res) => 
  accountingPurposesController.importData(req as AuthenticatedRequest, res))

// Bulk operations with rate limiting
router.post('/bulk/status', canUpdate('accounting-purposes'), bulkOperationLimit, validateSchema(bulkUpdateStatusSchema), (req, res) => 
...
router.get('/:id', canView('accounting-purposes'), validateSchema(accountingPurposeIdSchema), (req, res) => 
  accountingPurposesController.getById(req as AuthenticatedRequest, res))

router.put('/:id', canUpdate('accounting-purposes'), validateSchema(updateAccountingPurposeSchema), (req, res) => 
