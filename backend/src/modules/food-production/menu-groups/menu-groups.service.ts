...
export class MenuGroupsService {
  async list(companyId: string, pagination: { page: number; limit: number }, sort?: { field: string; order: string }, filter?: { is_active?: boolean; category_id?: string }) {
    const offset = (pagination.page - 1) * pagination.limit
    const { data, total } = await menuGroupsRepository.findAll(companyId, { limit: pagination.limit, offset }, sort, filter)
  ...
  async search(companyId: string, q: string, pagination: { page: number; limit: number }) {
    const offset = (pagination.page - 1) * pagination.limit
    const { data, total } = await menuGroupsRepository.search(companyId, q, { limit: pagination.limit, offset })
    const totalPages = Math.ceil(total / pagination.limit)
    return { data, pagination: { page: pagination.page, limit: pagination.limit, total, totalPages, hasNext: pagination.page < totalPages, hasPrev: pagination.page > 1 } }
  }

  async getById(id: string, companyId: string): Promise<MenuGroupWithCategory> {
    const group = await menuGroupsRepository.findById(id, companyId)
  ...
  async update(id: string, companyId: string, dto: UpdateMenuGroupDto, userId: string): Promise<MenuGroup> {
    const existing = await menuGroupsRepository.findById(id, companyId)
    if (!existing) throw new MenuGroupNotFoundError(id)
    ...
    await AuditService.log('UPDATE', 'menu_group', id, userId, existing, updated)
    return updated
  }
  ...
  async delete(id: string, companyId: string, userId: string): Promise<void> {
    ...
    await AuditService.log('DELETE', 'menu_group', id, userId, existing)
  }
