...
export class MenuCategoriesService {
  async list(companyId: string, pagination: { page: number; limit: number }, sort?: { field: string; order: string }, filter?: { is_active?: boolean }) {
    const offset = (pagination.page - 1) * pagination.limit
    const { data, total } = await menuCategoriesRepository.findAll(companyId, { limit: pagination.limit, offset }, sort, filter)
  ...
  async search(companyId: string, q: string, pagination: { page: number; limit: number }) {
    const offset = (pagination.page - 1) * pagination.limit
    const { data, total } = await menuCategoriesRepository.search(companyId, q, { limit: pagination.limit, offset })
    const totalPages = Math.ceil(total / pagination.limit)
    return { data, pagination: { page: pagination.page, limit: pagination.limit, total, totalPages, hasNext: pagination.page < totalPages, hasPrev: pagination.page > 1 } }
  }

  async getById(id: string, companyId: string): Promise<MenuCategoryWithCoa> {
    const category = await menuCategoriesRepository.findById(id, companyId)
  ...
  async update(id: string, companyId: string, dto: UpdateMenuCategoryDto, userId: string): Promise<MenuCategory> {
    const existing = await menuCategoriesRepository.findById(id, companyId)
    if (!existing) throw new MenuCategoryNotFoundError(id)
    ...
    await AuditService.log('UPDATE', 'menu_category', id, userId, existing, updated)
    return updated
  }
  ...
  async delete(id: string, companyId: string, userId: string): Promise<void> {
    ...
    await AuditService.log('DELETE', 'menu_category', id, userId, existing)
  }
