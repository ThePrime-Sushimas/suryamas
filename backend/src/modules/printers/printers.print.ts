...
class ReceiptBuilder {
  ...
  cmd(s: string): this {
    this.buffers.push(Buffer.from(s, "binary"));
    return this;
  }

  /** Push text line with LF */
  line(s: string): this {
    this.buffers.push(Buffer.from(s + LF, "binary"));
    return this;
  }

  /** Push empty line */
  lf(): this {
    this.buffers.push(Buffer.from(LF, "binary"));
    return this;
  }
...
function sep(w: number, char = "-"): string {
  return char.repeat(w);
}
function dsep(w: number): string {
  return "=".repeat(w);
}
    ...
    switch (row.type) {
      ...
      case "item":
        b.cmd(CMD.ALIGN_LEFT)
          .line(row.label)
          .line(cols(`   ${row.detail}`, row.amount, w));
        break;
      case "item-qty":
      ...
      case "total":
        b.cmd(CMD.ALIGN_LEFT)
          .cmd(CMD.BOLD_ON)
          .line(cols(row.label, row.amount, w))
          .cmd(CMD.BOLD_OFF);
        break;
      case "center":
        b.cmd(CMD.ALIGN_CENTER).line(row.text);
...
export function buildDocReceipt(data: PrintDocData): Buffer {
  ...
  if (data.footer) {
    ...
    rows.push({
      type: "center",
      text: `Printed: ${now.toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric", timeZone: "Asia/Jakarta" })} ${now.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}`,
    });
  }
...
export function buildDailyPrepOrderReceipt(
  data: DailyPrepOrderPrintData,
): Buffer {
  const now = new Date();
  const defaultFooter = `Printed: ${now.toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric", timeZone: "Asia/Jakarta" })} ${now.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}`;
...
/** Thermal Bukti Stock Transfer — transfer antar gudang/cabang. */
export function buildStockTransferReceipt(
  ...
  const rows: ReceiptRow[] = [
    { type: "title", text: "SURYAMAS" },
    { type: "subtitle", text: "Stock Transfer" },
    { type: "double-separator" },
    ...data.header.map((h) => ({
      type: "kv" as const,
      key: h.key,
