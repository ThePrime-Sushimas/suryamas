...
export class PrintersController {
  list = async (req: Request, res: Response): Promise<void> => {
    try {
      const data = await printersService.list(req.context!.company_id)
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const data = await printersService.getById(params.id, req.context!.company_id)
  ...
  testConnection = async (req: Request, res: Response): Promise<void> => {
    try {
      const { params } = (req as ValidatedAuthRequest<typeof printerIdSchema>).validated
      const connected = await printersService.testConnection(params.id, req.context!.company_id)
  ...
  printPurchaseRequest = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await printersService.printPurchaseRequest(body.printer_id, params.id, body.line_ids, req.context!.company_id, req.user!.id)
